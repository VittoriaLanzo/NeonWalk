...
import SectionNavBar from '@/components/SectionNav';
import CustomCursor from '@/components/CustomCursor';
import HeroDistrict from '@/components/HeroDistrict';
import { RESEARCH, PERFORMANCE, EXPERIENCE } from '@/content/sections';
...
const NAV_DOTS = [
  ...
  { label: 'Research',    range: [0.15, 0.34] },
  { label: 'Projects',    range: [0.35, 0.51] },
  { label: 'Experience',  range: [0.52, 0.69] },
...
export default function Index() {
  ...
  return (
    <div ref={containerRef} className="relative bg-background" style={{ height: '600vh' }}>
      ...
      <Suspense fallback={<div style={{ position: 'fixed', inset: 0, background: '#050512' }} />}>
      ...
      ─────────────────────────────────────────────────────────────────── */}
      <ResearchPanel     visible={sp >= 0.09 && sp < 0.29} onEnter={() => navigate('/research')} />
      <PerformancePanel  visible={sp >= 0.29 && sp < 0.44} onEnter={() => navigate('/performance')} />
      <ExperiencePanel   visible={sp >= 0.44 && sp < 0.59} onEnter={() => navigate('/experience')} />
      ...
      <div style={{
        ...
        padding: '16px 24px',
        background: 'linear-gradient(to bottom, rgba(5,5,18,0.75) 0%, transparent 100%)',
        pointerEvents: 'none',
      }}>
      ...
      <SectionAdvanceHint visible={sp >= 0.07 && sp < 0.09} label="Research"   color="#6E6EFF" />
      <SectionAdvanceHint visible={sp >= 0.60 && sp < 0.68} label="Experience" color="#00D4FF" />
...
const T = {
  heading: { fontFamily: "'Syne', sans-serif", fontWeight: 700, color: '#F0F0F5' as const },
  mono:    { fontFamily: "'Syne', sans-serif" as const },
  body:    { fontFamily: "'Inter', sans-serif", color: '#8888AA' as const },
  muted:   { fontFamily: "'Inter', sans-serif", color: '#44445A' as const },
} as const;

function ResearchPanel({ visible, onEnter }: { visible: boolean; onEnter: () => void }) {
  ...
  return (
    <GlassPanel visible={visible} side="right" stallSide="left" color="#6E6EFF" onEnter={onEnter}>
      ...
      <h2 style={{ ...T.heading, fontSize: '20px', lineHeight: 1.25, marginBottom: '8px' }}>{paper.name}</h2>
      <div style={{ ...T.mono, fontSize: '9px', color: '#00FF88', letterSpacing: '0.15em', marginBottom: '10px' }}>
        ACCEPTED · {paper.venue}
      </div>
      <p style={{ ...T.body, fontSize: '12px', lineHeight: 1.7, marginBottom: '16px' }}>{paper.desc}</p>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', paddingTop: '14px', borderTop: '1px solid #1E1E2E' }}>
        {paper.stats.map(s => (
          <div key={s.label}>
            <div style={{ ...T.heading, fontSize: '18px', lineHeight: 1, marginBottom: '3px', color: '#6E6EFF' }}>{s.value}</div>
            <div style={{ ...T.muted, fontSize: '9px', lineHeight: 1.3, letterSpacing: '0.04em' }}>{s.label}</div>
          </div>
        ))}
      </div>
    </GlassPanel>
...
function PerformancePanel({ visible, onEnter }: { visible: boolean; onEnter: () => void }) {
  ...
  return (
    <GlassPanel visible={visible} side="left" stallSide="right" color="#00FF88" onEnter={onEnter}>
      ...
      <h2 style={{ ...T.heading, fontSize: '20px', lineHeight: 1.2, marginBottom: '14px' }}>
        {PERFORMANCE.headline.split('\n').map((l, i) => <span key={i}>{l}{i === 0 && <br />}</span>)}
      </h2>
      {merged.map(c => (
        <div key={c.id} style={{ paddingBottom: '9px', marginBottom: '9px', borderBottom: '1px solid #1E1E2E' }}>
          <div style={{ ...T.mono, fontSize: '9px', color: '#00FF88', letterSpacing: '0.18em', marginBottom: '3px' }}>MERGED · {c.repo} {c.pr}</div>
          <div style={{ ...T.body, fontSize: '11px', lineHeight: 1.5 }}>{c.metrics[0]} · {c.metrics[1]}</div>
        </div>
      ))}
      {inReview.map(c => (
        <div key={c.id} style={{ paddingBottom: '9px', marginBottom: '9px', borderBottom: '1px solid #1E1E2E' }}>
          <div style={{ ...T.mono, fontSize: '9px', color: '#FFB800', letterSpacing: '0.18em', marginBottom: '3px' }}>IN REVIEW · {c.repo} {c.pr}</div>
          <div style={{ ...T.body, fontSize: '11px', lineHeight: 1.5 }}>{c.metrics[0]}</div>
        </div>
      ))}
    </GlassPanel>
  );
...
function ExperiencePanel({ visible, onEnter }: { visible: boolean; onEnter: () => void }) {
  return (
    <GlassPanel visible={visible} side="right" stallSide="left" color="#00D4FF" onEnter={onEnter}>
      <SectionLabel label="Experience" />
      <h2 style={{ ...T.heading, fontSize: '20px', lineHeight: 1.2, marginBottom: '18px' }}>
        {EXPERIENCE.headline.split('\n').map((l, i) => <span key={i}>{l}{i === 0 && <br />}</span>)}
      </h2>
      {EXPERIENCE.timeline.map((entry, i) => (
        <div key={i} style={{ display: 'grid', gridTemplateColumns: '44px 1fr', gap: '12px', marginBottom: '12px' }}>
          <div style={{ ...T.muted, fontSize: '10px', letterSpacing: '0.08em', paddingTop: '2px' }}>{entry.year}</div>
          <div>
            <div style={{ ...T.heading, fontSize: '12px', marginBottom: '2px' }}>{entry.role}</div>
            <div style={{ ...T.body, fontSize: '10px', color: '#00D4FF', letterSpacing: '0.04em' }}>{entry.org}</div>
          </div>
        </div>
      ))}
    </GlassPanel>
  ...
  return (
    <div style={{
      ...
      <form onSubmit={handleSubmit}>
        ...
        <textarea placeholder="What are you working on?"value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} style={{ ...inputStyle, height: '90px', resize: 'none' }} onFocus={e => { e.target.style.borderColor = '#6E6EFF'; }} onBlur={e => { e.target.style.borderColor = '#1E1E2E'; }} />
