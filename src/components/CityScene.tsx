...
const LOD_CHECK_INTERVAL = 8;   // recompute LOD every Nth frame, staggered per building
...
function Building({ data, brickMap }: { data: BuildingData; brickMap: THREE.Texture }) {
  ...
  const framePhase = useRef(Math.floor(Math.random() * LOD_CHECK_INTERVAL));
  ...
  useFrame(({ camera, clock }) => {
    ...
    // Throttle the LOD distance check — staggered across buildings so they
    // don't all recompute on the same frame. Squared distance avoids sqrt.
    if ((frameCount.current++ + framePhase.current) % LOD_CHECK_INTERVAL === 0) {
      const dx = camera.position.x - pos[0];
      const dz = camera.position.z - pos[2];
      const distSq = dx * dx + dz * dz;
      const newLod: 0 | 1 | 2 = distSq < LOD_NEAR_SQ ? 0 : distSq < LOD_MID_SQ ? 1 : 2;
...
function CameraController({ scrollProgress }: { scrollProgress: number }) {
  ...
  const eased = useRef(scrollProgress);
  useFrame((_, delta) => {
    // Frame-rate-independent damping: the camera glides toward the scroll
    // target each frame instead of snapping to discrete scroll events. This
    // is what makes the flythrough smooth even when scroll input is chunky.
    eased.current = THREE.MathUtils.damp(eased.current, scrollProgress, 10, Math.min(delta, 0.05));
...
/* ─── Data Conduit Lines ───────────────────────────────────────────────────
   Two glowing traces running the full length of the road, one on each
   sidewalk edge. Static geometry — zero useFrame, one shared pulse.
   ─────────────────────────────────────────────────────────────────────── */

function DataConduits() {
  ...
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    /* Line pulse */
    const base = 2.4 + Math.sin(t * 0.8) * 0.8;
    if (matL.current) matL.current.emissiveIntensity = base;
    if (matR.current) matR.current.emissiveIntensity = base + Math.sin(t * 0.6 + 1.1) * 0.6;
    /* Moving dot — travels 0→-200 over 4 s, loops */
    const zL = 15 - ((t * 50) % 215);
    const zR = 15 - ((t * 50 + 107) % 215);
    if (dotL.current) dotL.current.position.z = zL;
    if (dotR.current) dotR.current.position.z = zR;
  });
  ...
  return (
    <>
      {/* Left conduit — violet */}
      <mesh position={[-4.6, 0.01, -length / 2 + 20]}>
        <boxGeometry args={[0.07, 0.015, length]} />
        <meshStandardMaterial ref={matL} color="#000" emissive="#6E6EFF" emissiveIntensity={2.4} />
      </mesh>
      {/* Moving pulse dot — left */}
      <mesh ref={dotL} position={[-4.6, 0.02, 0]}>
        <boxGeometry args={[0.14, 0.025, 2.5]} />
        <meshStandardMaterial color="#000" emissive="#6E6EFF" emissiveIntensity={8} />
      </mesh>

      {/* Right conduit — green */}
      <mesh position={[4.6, 0.01, -length / 2 + 20]}>
        <boxGeometry args={[0.07, 0.015, length]} />
        <meshStandardMaterial ref={matR} color="#000" emissive="#00FF88" emissiveIntensity={2.4} />
      </mesh>
      {/* Moving pulse dot — right */}
      <mesh ref={dotR} position={[4.6, 0.02, 0]}>
        <boxGeometry args={[0.14, 0.025, 2.5]} />
        <meshStandardMaterial color="#000" emissive="#00FF88" emissiveIntensity={8} />
      </mesh>

      {/* Center dashes — cyan */}
      {Array.from({ length: 18 }, (_, i) => (
        <mesh key={i} position={[0, 0.009, 10 - i * 12]}>
          <boxGeometry args={[0.05, 0.012, 4.5]} />
          <meshStandardMaterial color="#000" emissive="#00D4FF" emissiveIntensity={1.4} transparent opacity={0.7} />
        </mesh>
      ))}
    </>
  );
...
const STALL_DESCRIPTIONS: Record<string, string> = {
  research:    'ML research · EEML 2026',
  performance: 'Open-source projects',
  experience:  'Timeline & background',
  about:       'Who I am & how to reach me',
};

function NavigationStalls({ onStallClick }: { onStallClick: (id: string) => void }) {
  const stalls = useMemo(() => [
    { id: 'research',    label: 'RESEARCH',    z: -20,  side: -1, color: '#6E6EFF' },
    { id: 'performance', label: 'PROJECTS',    z: -55,  side:  1, color: '#00FF88' },
    { id: 'experience',  label: 'EXPERIENCE',  z: -90,  side: -1, color: '#00D4FF' },
    { id: 'about',       label: 'ABOUT',       z: -130, side:  1, color: '#FF2D78' },
  ], []);
  ...
  return (
    <>
      {stalls.map((stall) => (
        ...
        >
          {/* ── Full-panel invisible hit overlay ── */}
          <mesh
            position={[0, 1.15 * S, 0.2 * S]}
            ...
            renderOrder={999}
          >
            <boxGeometry args={[2.0 * S, 2.6 * S, 0.6]} />
...
function StreetProps({ isMobile }: { isMobile: boolean }) {
  const props = useMemo(() => {
    ...
    return { manholes, puddles, debris, vendorStalls };
  }, [isMobile]);
  ...
  return (
    <>
      ...
      {/* Puddles — high-metalness mirror without extra render passes */}
      {props.puddles.map((p, i) => (
        <mesh key={`pud${i}`} position={p.pos} rotation={[-Math.PI / 2, 0, 0]} scale={[p.sx, p.sz, 1]}>
          <circleGeometry args={[1, 20]} />
          <meshStandardMaterial color="#020210" metalness={0.95} roughness={0.05} transparent opacity={0.88} depthWrite={false} />
        </mesh>
      ))}
...
/* ─── City Environment ───
   Memoized: scrollProgress lives in the parent and changes on every scroll
   tick, but nothing in the world depends on it (only CameraController does).
   memo + a stable onStallClick callback skips reconciling the whole scene. */
const CityEnvironment = memo(function CityEnvironment({ onStallClick }: { onStallClick: (id: string) => void }) {
  ...
  return (
    <>
      ...
      <RainParticles count={isMobile ? 30 : 80} />
      <Stars radius={200} depth={60} count={isMobile ? 300 : 600} factor={4} saturation={0} />
      <CityGlowDome />
