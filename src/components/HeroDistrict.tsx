...
export default function HeroDistrict() {
  ...
  // Scroll to a fraction (0–1) of the full scroll range. The page is 600vh and
  // scrollProgress drives the camera, so a fraction maps to a point on the street.
  const scrollToProgress = useCallback((fraction: number) => {
    const docHeight = document.documentElement.scrollHeight - window.innerHeight;
    window.scrollTo({ top: docHeight * fraction, behavior: 'smooth' });
  }, []);
      ...
      <div className="text-center max-w-4xl">
        ...
        <div className="flex flex-col sm:flex-row gap-5 justify-center mt-16">
          ...
          <button
            ...
            className="font-body text-[13px] tracking-wide text-text-secondary hover:text-foreground transition-colors duration-200 bg-transparent border-none cursor-pointer"
          >
            Get in touch
          </button>
        </div>
