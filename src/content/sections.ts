...
Sestara is an AI-powered edtech platform that turns any exam goal into a structured, trackable study journey — with the precision of a private tutor built into every interaction. It generates a complete, specific curriculum for any exam, tracks every topic in real time, quizzes students with explanations, and deploys an AI companion that knows exactly where they stand. The product is live at sestara.lovable.app. Vittoria led the full frontend and design: she wrote the React components, built the design system from scratch, implemented all animations, and drove every product decision on the UI layer. Harshit Singh led the backend.
...
The hardest frontend problem on Sestara was not the component count — it was maintaining a coherent, emotionally consistent experience across four distinct modules that each behave differently but must feel like one product. Every transition, every state change, every piece of AI output had to land with the same register. That required building the design system and the component architecture simultaneously, so the visual language and the code structure reinforced each other from the start.
...
The emotional register of StudyBuddy was engineered with the same precision as its knowledge scope. The design intent is specific: the assistant checks in, notices when a student is three weeks behind schedule, and addresses it directly rather than just answering the surface question. The goal — the thing the system prompt is written toward — is that the difference between a student who quits and one who pushes through is rarely intelligence. It is having someone in their corner at 2 AM the night before the exam. Whether that holds in practice is something only students can confirm. But the prompt encodes that person deliberately: their tone, their awareness, their conditions for offering encouragement versus challenge.
